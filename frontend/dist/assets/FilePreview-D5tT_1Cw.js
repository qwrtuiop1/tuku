import { a as api, f as formatFileSize, _ as _export_sfc, e as getFilePreviewUrl, h as formatTime, c as getCachedImageUrl } from "./index-Bvf1lGuu.js";
/* empty css                    */
  ...
  setup(__props, { emit: __emit }) {
    ...
    async function detectMotionPhoto(file) {
      ...
      try {
        ...
        const windowSize = Math.min(2 * 1024 * 1024, file.size);
        const buf = await readChunk(file.size - windowSize, windowSize);
        ...
        if (eoiAfter < 0 || eoiAfter >= bytes.length - 8) return false;
        const after = bytes.subarray(eoiAfter);
        let pos = 0;
        while (true) {
          let idx = -1;
          for (let j = pos; j <= after.length - 4; j++) {
            if (after[j] === 102 && after[j + 1] === 116 && after[j + 2] === 121 && after[j + 3] === 112) {
              idx = j;
              break;
            }
          }
          if (idx === -1) break;
          if (idx >= 4) {
            const boxLen = after[idx - 4] << 24 | after[idx - 3] << 16 | after[idx - 2] << 8 | after[idx - 1];
            const remaining = after.length - (idx - 4);
...
const FileUploader = /* @__PURE__ */ _export_sfc(_sfc_main$2, [["__scopeId", "data-v-911921bb"]]);
...
//# sourceMappingURL=FilePreview-D5tT_1Cw.js.map
