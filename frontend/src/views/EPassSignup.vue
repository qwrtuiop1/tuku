<template>
  <div class="login-container epass-signup">
    <div class="top-nav">
          ...
          <el-form :model="form" :rules="rules" ref="formRef" label-position="top">
            <el-form-item label="用户名" prop="username">
              <div class="row-inline">
                ...
                <el-button v-if="profile.nickname" @click="useNickname" class="use-nickname-btn" type="default">使用E通行证昵称</el-button>
              </div>
            </el-form-item>
            ...
            <el-form-item>
              <el-button class="submit-btn" type="primary" :loading="submitting" @click="submit">完成注册</el-button>
            </el-form-item>
          </el-form>
        </div>
      </div>
  ...
  return await new Promise<boolean>((resolve) => {
    ...
    const onSuccess = async () => {
      ...
      try {
        ...
        const resp = await api.post('/auth/captcha/validate', { lot_number, captcha_output, pass_token, gen_time, captcha_id: geetestCaptchaId })
        if (resp?.data?.success || resp?.data?.result === 'success') return resolve(true)
        ElMessage.error(resp?.data?.message || resp?.data?.reason || '人机验证失败'); resolve(false)
      } catch {
        ElMessage.error('人机验证服务异常，请稍后重试'); resolve(false)
      }
    }
    try {
...
const { startEmailCodeCooldown, sendEmailCodeWithHuman } = useEmailCode({ defaultCooldownSeconds: 60, runHuman: verifyHuman })
...
onMounted(() => {
  ...
  profile.value.nickname = url.searchParams.get('nickname') || ''
  profile.value.avatar = url.searchParams.get('avatar') || ''
  const preEmail = url.searchParams.get('email') || ''
  if (preEmail) form.value.email = preEmail
})

const useNickname = () => { if (profile.value.nickname) form.value.username = profile.value.nickname }
...
const submit = async () => {
  ...
  try {
    ...
    const res = await api.post('/auth/epass/complete-signup', {
...
.epass-profile { display:flex; align-items:center; gap:12px; margin-bottom:12px; }
.epass-info .title { font-size:20px; font-weight:600; color:#f3f4f6; }
.epass-info .sub { font-size:13px; color:#9ca3af; }
.row-inline { display:flex; gap:8px; align-items:center; width:100%; }
.code-row > :deep(.el-input) { flex: 1 1 auto; }
.send-code-btn,
.use-nickname-btn { flex: 0 0 128px; height: 36px; padding: 0 14px; border: 1px solid #2a2f3a; background: linear-gradient(180deg, #ffffff, #f5f6f8); color: #111827; border-radius: 8px; font-size: 14px; }
.send-code-btn:hover,
.use-nickname-btn:hover { border-color: #343a46; background: linear-gradient(180deg, #f5f6f8, #eceff3); }
.send-code-btn:focus,
.use-nickname-btn:focus { outline: none; }
...
.submit-btn { width: 100%; height: 40px; border-radius: 8px; background: linear-gradient(180deg, #2d2f33, #111315); color: #ffffff; border: 1px solid #2d2f33; }
.submit-btn:hover { background: linear-gradient(180deg, #34363b, #14161a); }
.submit-btn:focus { outline: none; }

@media (max-width: 1280px) { .login-content { gap: 20px; padding: 0 12px; } .login-box { flex-basis: 560px; } .form-inner { max-width: 500px; } }
@media (max-width: 1024px) { .login-content { flex-direction: column; align-items: center; gap: 16px; } .login-box { flex-basis: auto; width: 100%; max-width: 720px; } .form-inner { width: 85%; max-width: 560px; } .info-panel { display: none !important; } .floating-shape { width: 180px; height: 180px; filter: blur(36px); } }
@media (max-width: 768px) { .top-nav { padding: 12px 14px; } .login-content { margin: 16px auto; padding: 0 10px; } .login-box { padding: 18px; border-radius: 14px; } .form-inner { width: 100%; max-width: 100%; } .epass-profile .title { font-size: 18px; } .epass-profile .sub { font-size: 12px; } .row-inline { gap: 6px; } .agreement-checkbox { font-size: 12px; } .terms-link { margin: 0 1px; } .send-code-btn, .use-nickname-btn { flex-basis: 120px; height: 34px; font-size: 13px; } .floating-shape { width: 150px; height: 150px; filter: blur(32px); opacity: 0.12; } }
@media (max-width: 480px) { .login-box { padding: 14px; border-radius: 12px; } .form-inner { max-width: 100%; } .send-code-btn, .use-nickname-btn { flex-basis: 112px; height: 32px; font-size: 12px; padding: 0 10px; } .row-inline { gap: 6px; } .agreement-checkbox { font-size: 12px; line-height: 1.15; } .agreement-checkbox :deep(.el-checkbox__input) { margin-right: 4px; } .logo-text { display: none; } .floating-shape { display: none; } }
</style>
