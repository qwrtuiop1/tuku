...
class CiService {
  constructor(config) {
    ...
    // 解析 cosHost
    if (config.cosHost) {
      // 检查是否是完整的 COS 域名格式 (xxx.cos.ap-xxx.myqcloud.com)
      if (/^[^.]+\.cos\./.test(config.cosHost)) {
        // 完整的 COS 域名，提取 bucket 名称（第一部分）
        const firstPart = config.cosHost.split('.')[0];
        if (firstPart && firstPart !== this.bucket) {
          // bucket 名称与第一部分不同，优先使用 cosHost
          this.cosHost = config.cosHost;
        } else {
          // 第一部分就是 bucket 名称
          this.bucket = firstPart;
          this.cosHost = config.cosHost;
        }
      } else {
        // 不是完整域名，当作 bucket 名称处理
        this.bucket = config.cosHost;
        this.cosHost = `${this.bucket}.cos.${this.region}.myqcloud.com`;
      }
    } else {
      this.cosHost = `${this.bucket}.cos.${this.region}.myqcloud.com`;
    }

    // 确保 bucket 名称正确（提取不带 -appid 后缀的部分用于查询）
    // COS bucket 格式：bucket-appid
    const bucketAppIdMatch = this.bucket.match(/^(.+?)-\d+$/);
    this.bucketBase = bucketAppIdMatch ? bucketAppIdMatch[1] : this.bucket;
    this.appId = config.appId || (this.bucket.match(/-\d+$/) ? this.bucket.match(/-\d+$/)[0].slice(1) : '');
  ...
  /**
   * 仅当 COS 桶名、密钥为真实配置时才应 302 / 返回签名 URL。
   * 占位符（如 bucket 名仅为 "bucket"、缺少 -AppID）会生成错误域名，浏览器侧出现 400 + CORS。
   */
  canUseSignedUrls() {
    if (!this.enable) return false;
    const sid = String(this.secretId || '');
    const sk = String(this.secretKey || '');
    if (!sid || !sk) return false;
    const sidL = sid.toLowerCase();
    if (sidL.includes('your_') || sidL.includes('example') || sid === 'your_secret_id_here') return false;
    const skL = sk.toLowerCase();
    if (skL.includes('your_') || sk === 'your_secret_key_here') return false;

    const b = String(this.bucket || '').trim();
    if (!b) return false;
    const bL = b.toLowerCase();
    if (bL === 'bucket' || bL === 'your-bucket-name-1250000000' || /^your-/.test(bL)) return false;
    // 标准 COS 桶：名称-AppID（末尾为数字）
  ...
  getSignedUrl(key, expiresIn = 3600) {
    ...
    const encodedKey = encodeURIComponent(keyWithoutSlash);
    ...
    console.log('[CiService] Signing URL:', {
      cosHost: this.cosHost,
      key: keyWithoutSlash,
      exp: new Date(exp * 1000).toISOString(),
      bucket: this.bucket,
      bucketBase: this.bucketBase,
      secretIdPrefix: this.secretId ? this.secretId.substring(0, 4) + '****' : 'empty'
    });

    // 返回带签名的 URL
    return `https://${this.cosHost}/${encodedKey}?sign=v2&exp=${exp}&ak=${this.secretId}&sig=${signature}`;
  }
