...
// CI 启用检查（带缓存）
let ciEnabledCache = null;
let ciEnabledCacheTs = 0;

async function isCiEnabled() {
  const now = Date.now();
  if (ciEnabledCache !== null && (now - ciEnabledCacheTs) < 5000) {
    return ciEnabledCache;
  }
  try {
    const ci = await ciService.getInstance();
    ciEnabledCache = ci.enable;
    ciEnabledCacheTs = now;
    return ci.enable;
  } catch (_) {
    ciEnabledCache = false;
    ciEnabledCacheTs = now;
    return false;
  }
}
...
// 清除 CI 启用缓存
async function clearCiEnabledCache() {
  ciEnabledCache = null;
  ciEnabledCacheTs = 0;
}
...
router.get('/preview/:id', authenticateToken, asyncHandler(async (req, res) => {
  ...
  if (isCosFile) {
    ...
    // CI 未启用，使用默认 COS URL
    const cosHost = process.env.CI_COS_HOST || `${process.env.CI_BUCKET || 'bucket'}.cos.${process.env.CI_REGION || 'ap-guangzhou'}.myqcloud.com`;
    return res.redirect(302, `https://${cosHost}/${file.file_path}`);
  }
...
router.get('/:id/signed-url', authenticateToken, asyncHandler(async (req, res) => {
  ...
  if (useCi) {
    const ci = await ciService.getInstance();
    if (ci.enable && file.file_path && file.file_path.startsWith('users/')) {
      // COS 文件，生成签名 URL
